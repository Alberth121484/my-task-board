      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
        {STATUSES.map(({ id, label: statusLabel, bgColor, icon }) => (
          <button
            ...
            type="button"
            onClick={() => onChange(id)}
            className={`flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-task border-2 transition-all duration-200
                       ${value === id 
                         ? 'border-task-blue bg-blue-50' 
                         : 'border-task-gray-light hover:border-gray-300'
                       }`}
          >
            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-icon flex items-center justify-center ${bgColor}`}>
              <img src={icon} alt={statusLabel} className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <span className="text-xs sm:text-sm font-medium text-gray-700">{statusLabel}</span>
            {value === id && (
              <div className="ml-auto">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-task-blue" fill="currentColor" viewBox="0 0 20 20">
